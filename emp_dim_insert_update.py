...
def join_and_detect_new_or_changed_rows():
    ...
    emp_detail = pd.read_csv('s3://staging.emp.data/mariam_emp_details.csv')
    emp_detail.drop(columns='Unnamed: 0', inplace=True)
    emp_sal = pd.read_csv('s3://staging.emp.data/mariam_emp_sal.csv')
    ...
    conn = connect(
            user='mariam',
            password='rujwaq-tuzxu7-godvuD',
            account='oob12316.us-east-1'
        )
